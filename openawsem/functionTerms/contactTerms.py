...
def contact_term_reference(oa, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=".", burialPartOn=False, withExclusion=True, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45):
    import pandas
    if isinstance(k_contact, float) or isinstance(k_contact, int):
...
#0505 analysis_energy_fibril vs. monomer
def interface_contact_term(oa, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=False, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45):
    if parametersLocation is None:
        parametersLocation=openawsem.data_path.parameters
    if isinstance(k_contact, float) or isinstance(k_contact, int):
    ...
    contact.addPerParticleParameter("resId")
    contact.addPerParticleParameter("isCb")
    contact.addPerParticleParameter("isMonomer") #new add
    contact.addPerParticleParameter("isFibril")
    ...
    fibril_range = range(0, 384) #residues of fibril
    monomer_range = range(384, 416) #residues of monomer

    for i in range(oa.natoms):
        res_idx = oa.resi[i]    #map each atom to its corresponding residue.
        is_m = 1.0 if res_idx in monomer_range else 0.0  #determine whether the atom is a monomer or a fibril based on the residue range.
        is_f = 1.0 if res_idx in fibril_range else 0.0
        
        contact.addParticle([gamma_se_map_1_letter[seq[oa.resi[i]]], oa.resi[i], int(i in cb_fixed), is_m, is_f])
    ...
    if z_dependent:
        ...
        # mediated and direct term (write separately may lead to bug)
        contact.addEnergyTerm(f"-(isMonomer1*isFibril2 + isFibril1*isMonomer2)*isCb1*isCb2*res_table({inMembrane}, resId1, resId2)*{k_contact}*\
                                (gamma_ijm({inMembrane}, resName1, resName2)*theta+thetaII*(sigma_water*water_gamma_ijm({inMembrane}, resName1, resName2)+\
                                sigma_protein*protein_gamma_ijm({inMembrane}, resName1, resName2)));\
                                sigma_protein=1-sigma_water;\
                                theta=0.25*(1+tanh({eta}*(r-{r_min})))*(1+tanh({eta}*({r_max}-r)));\
...
#0505 analysis_energy_fibril vs. monomer direct only
def interface_contact_direct_term(oa, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=False, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45):
    if parametersLocation is None:
        parametersLocation=openawsem.data_path.parameters
    if isinstance(k_contact, float) or isinstance(k_contact, int):
...
#0505 analysis_energy_fibril vs. monomer water only
def interface_contact_water_term(oa, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=False, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45):
    
    print(">>> USING interface_contact_water_term") 
...
#0505 analysis_energy_fibril vs. monomer protein only
def interface_contact_protein_term(oa, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=None, burialPartOn=True, withExclusion=False, forceGroup=22,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45):
    
    print(">>> USING interface_contact_protein_term")
    ...
    if z_dependent:
        ...
        # protein term 
        contact.addEnergyTerm(f"-(isMonomer1*isFibril2 + isFibril1*isMonomer2)*isCb1*isCb2*res_table({inMembrane}, resId1, resId2)*{k_contact}*\
                        (thetaII*sigma_protein*protein_gamma_ijm({inMembrane}, resName1, resName2));\
                        sigma_protein=1-sigma_water;\
                        thetaII=0.25*(1+tanh({eta}*(r-{r_minII})))*(1+tanh({eta}*({r_maxII}-r)));\
                        sigma_water=0.25*(1-tanh({eta_sigma}*(rho1-{rho_0})))*(1-tanh({eta_sigma}*(rho2-{rho_0})))",
                        CustomGBForce.ParticlePair)
